...
public class Application implements producto {
	...
	public static void main(String[] args) {
		...
		String lista = shopping.stream().filter(x -> x.name.startsWith("C"))
				.sorted((x, y) -> x.name.compareToIgnoreCase(y.name)).map(x -> x.name).collect(Collectors.joining(","));
